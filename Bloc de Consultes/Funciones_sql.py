from Funciones_Login import conectar_bd, cerrar_bd
import psycopg2
import os
import time
...
#1
def informacion_de_la_planta(planta):
    conn, cur = conectar_bd()
...
#2
def Informe_nombre_visites_ateses():
    '''Totes les visites'''
    conn, cur = conectar_bd()
    if conn and cur:
        try:
...
def Informe_nombre_visites_ateses_per_data(data):
    '''Totes les visites'''
    conn, cur = conectar_bd()
    if conn and cur:
        try:
...
def informe_personal():
    ...
    if conn and cur:
        try:
            ...
            cerrar_bd(conn, cur)
#3
def Ranking_metges_mes_pacients():
...
def Malalties_mes_comunes():
    ...
    if conn and cur:
        try:
            ...
            cerrar_bd(conn, cur)  # Corrección: Asegúrate de que esta línea esté correctamente escrita
