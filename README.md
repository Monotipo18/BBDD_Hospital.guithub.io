...
## Contingut
- [Connectivitat i Login](#connectivitat-i-login)
- [Esquema BD Relacional](#esquema-bd-relacional)
- [Esquema de Seguretat](#esquema-de-seguretat)
- [Bloc de Manteniment](#bloc-de-manteniment)
- [Alta Disponibilitat](#alta-disponibilitat)
- [Bloc de Consultes](#bloc-de-consultes)
- [Dummy Data](#dummy-data)
- [Exportació de Dades](#exportaci%C3%B3-de-dades)
...
A part de les tasques anteriorment mencionades, s'ha de tenir en compte que en guardar les dades de login i registre s'està posant en risc la seguretat d'aquestes. És per això que s'ha d'aplicar alguna mesura de seguretat per evitar problemes.

> [! WARNING]
> És molt important mencionar que no es recomana gens guardar dades d'inici de sessió, a causa dels riscos que comporta.
...
- 'Pwinput': Utilitzada per emmascarar els caràcters escrits en una cadena de text.
...
- [Bloc de Connectivitat i Login](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/blob/main/Login/Documentacion_Login.md)
...
En aquest cas, com que s'està dissenyant una BD per a un hospital, caldrà guardar tota mena de dades, des d'informació de pacients o metges,
...
Per obtenir una explicació molt més detallada de què contenen cada Entitat i els seus atributs, es recomana visitar l'apartat de:
...
els diferents permisos que tenen sobre els objectes (dades) de la BD. És per això que, a tall de resum, els rols que existiran a la BD són els següents:
...
[Usuaris, Grups i Permisos](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/tree/main/Esquema%20de%20seguretat/Matriu%20de%20Seguretat)
...
> [! NOTE]
> El Script '''.sql''' també es troba en el mateix bloc que la documentació de permisos (l'enllaç adjuntat), també hi ha una taula en Excel per entendre millor els permisos.

#### SSL-TLS

Primer de tot, cal aclarir una cosa: SSL, avui dia no s'utilitza, ja que el protocol TLS (Transport Layer Security) el va substituir el 1999. El dia d'avui, 12 d'abril de 2024, és l'estàndard en la seva versió TLS 1.3. No obstant això, el terme SSL es continua utilitzant per referir-se a connexions segures.

Un cop els rols i els permisos estiguin creats, cal implementar una encriptació del tràfic de la BD. Per què? Per la senzilla raó que SSL permet xifrar el trànsit de dades, cosa que és molt convenient, ja que si les dades no estan xifrades, un intermediari podria interceptar-les (Man in the Middle), i això en una BD d'un hospital és totalment impensable.
...
Un cop se sap amb quin tipus de dades es treballarà, es recomana fer un estudi per saber a quines dades de la BD s'aplicarà el Data Masking tenint en compte els apartats anteriorment esmentats.
...
- **Requisits Baixos: ** Aquests documents són necessaris i obligatoris, però poc rellevants, per tant, tindran poca necessitat d'encriptació. Utilitzarem Data Masking mínimament i l'encriptació SSL.

Cal esmentar que l'hospital implementa diferents tècniques per assegurar, protegir i minimitzar l'accés a tota mena de dades de l'hospital mitjançant mesures de seguretat tècniques, anàlisi de riscos, mesures de seguretat organitzatives, gestió d'accés, protecció de la infraestructura física, revisions i actualitzacions, i finalment en Procediments de Seguretat en Cas d'Incidents.
...
- [Legislació AGPD](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/blob/main/Esquema%20de%20seguretat/Documentacion%20RGPD/Documento%20de%20seguridad.md)

### Bloc de Manteniment

El bloc de manteniment és un apartat per millorar l'anterior apartat de Connectivitat i Login, per a realitzar això es planteja dur a terme les següents tasques i implementar-les en l'aplicació:

1. Donar d'alta nou personal al centre (metge/sa, infermer/a, administratiu/va, neteja...)
2. Donar d'alta nous pacients.
3. Pel personal d'infermeria, saber si depèn d'un metge/sa o bé és de planta.
4. Per un determinat dia, saber per a cada quiròfan, les operacions que hi ha previstes, el pacient a operar, l'hora, el metge/sa que les farà i el personal d'infermeria que intervindrà.
5. Per un determinat dia, saber les visites que hi ha planificades, l'hora d'entrada, el metge/sa i el pacient.
...
9. Donat un/a metge/sa saber les visites i les operacions que té programades i les seves hores disponibles.
...
- '''Dotenv''': És una eina útil en el món de Python per gestionar les variables d'entorn d'una manera senzilla i eficaç.
...
> [! TIP]
> Malgrat que aquest apartat encara no conté la versió final de l'aplicació, es recomana fer-ho per evitar confusions.

### Alta disponibilitat

#### Infraestructura Hardware

A causa de les característiques de la Base de dades de l'hospital (quantitat d'informació emmagatzemada) s'han considerat una quantitat diferent d'opcions i s'ha decidit que l'opció més adient per a les necessitats de l'hospital és la següent:

S'ha escollit el servidor Dell PowerEdge R740XD2 que té unes característiques de maquinari que s'ha aproximarien al necessari per al bon funcionament del sistema. S'ha utilitzat la pàgina web https://www.renewtech.es/dell-poweredge-r740xd2-configure-to-order.html per a personalitzar el servidor segons les necessitats.

Les característiques aproximades són, una CPU de 12/16 nuclis, 64/128 GB de ram, 50/100 TB de emmagatzematge i sobretot que tingui una garantia general en cas de fallada.
...
>
- [Hardware Escollit](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/tree/main/Esquema%20d'alta%20disponibilitat/Infraestructura%20Hardware)
...
- [Replicació BD](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/tree/main/Esquema%20d'alta%20disponibilitat/Replicacio)

#### Backups i restauració

S'ha decidit crear scripts en Bash per fer còpies de seguretat tant localment com al núvol, utilitzant el servei d'emmagatzematge OneDrive. A més, s'ha configurat un crontab per executar els scripts corresponents diàriament, i finalment s'ha creat un script per a la restauració de la base de dades de manera més ràpida i senzilla.
...
2. Emmagatzematge Local: Es guardaran un total de 5 còpies de seguretat localment, eliminant la més antiga quan s'arribi al límit.
3. Emmagatzematge en el núvol: Es crearà una còpia de seguretat al núvol (OneDrive) cada dia que es realitzi una còpia incremental.

El procés de còpia de seguretat s'automatitza mitjançant l'eina crontab. S'han creat els seguents scripts:

- Script de Backup Lògic pujat al núvol (copia_local_nube_tarde.sh): Realitza la còpia local i al mateix temps puja al núvol, durant el canvi de torn a les 2 PM (OneDrive).
- Script de Backup Lògic pujat al núvol (copia_local_nube_noche.sh): Realitza la còpia local i al mateix temps puja al núvol, en horari nocturn 00:00 AM (OneDrive).
...
- [Backups i Restauració](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/tree/main/Esquema%20d'alta%20disponibilitat/Backups%20i%20restauracio)

A part de la documentació, es troben els 2 scripts, un per configurar un script de còpia de seguretat al núvol i local al mateix temps i un altre script per restaurar la còpia més recent realitzada.
...
Per trobar els següents apartats realitzats, que són una millora de l'anterior apartat que millorava l"Aplicació", es troben en el següent enllaç:

- [Bloc de consultes](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/tree/main/Bloc%20de%20Consultes)

### Dummy Data

Per posar a prova la Base de dades s'ha decidit crear dades fictícies de la següent manera:

Per crear aquestes dades fictícies per validar el rendiment del sistema, es crearan (100.000 visites, 50.000 pacients, 100 metges, 200 infermeres, 100 persones de neteja i 50 persones d'administració), i es crearan els índexs que corresponguin, triant el més adequat per a cada taula. Una petita mostra d'aquesta informació estarà en alfabet ciríl·lic.
...
Per l'exportació de dades s'ha proposat la descarrega de totes les visites que hi ha hagut entre dues dates on figuri un identificador de visita, dia, metge que ha atès i les dades de pacient,
la descàrrega s'ha de fer amb XML i El document ha de gravar-se identat (amb tabulacions).

Para poder realitzar el següent apartat és necessari importar les següents llibreries de '''python''' ( ja venen integrades):
'''
import xml.etree.ElementTree as ET
import xml.dom.minidom
'''
Un cop importades les llibreries es recomana seguir la documentació d'Exportació de dades per configurar-ho correctament, a més en aquest apartat es troben els codis finalitzats d'exportació:

- [Exportació de Dades](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/tree/main/Bloc%20d'exportacio%20de%20dades)

#### Informes en PowerBi

Per poder utilitzar Power BI, s'ha d'instal·lar a través de la pàgina web de Microsoft en el següent enllaç:

- [PowerBi Descarrega](https://www.microsoft.com/es-es/download/details.aspx?id=58494)
...
> "Error Certificado Remoto no Vàlido". En cas que succeeixi, es recomana fer el següent:

#### Configuració Alternativa

En cas que la configuració convencional no funcioni, es pot optar per utilitzar eines de tercers que el mateix Microsoft permet i recomana, una d'aquestes és l'eina ODBC. Aquesta eina actua com a intermediari entre la Base de Dades i el Power BI i permet la connexió sense que aparegui l'error de **Certificado Remot no Vàlido**.
...
En el següent enllaç es troba el Manual d'Usuari per poder utilitzar l'aplicació sense cap mena de dificultat:

- [Manual d'Usuari](https://github.com/Monotipo18/BBDD_Hospital.guithub.io/blob/main/Login/Connexio%20al%20Programa/GHEN%20User%20Manual.pdf)

## Extra 1: Logs BD

Per motius de seguretat s'ha decidit implementar un sistema de logs a la Base de Dades per saber si els mateixos treballadors fan consultes de pacients o d'un altre tipus quan no haurien de fer-ho.
...
> [! CAUTION]
> Projecte elaborat per: Alex, ,Manuel,Victor i Unai.
